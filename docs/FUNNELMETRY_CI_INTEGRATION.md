...
generates `integration-plan.json` and `integration.patch` outside the checkout,
checks the generated-file allowlist, resolves private packages, builds/type-checks
the host, then commits to and pushes only that target branch. It uploads an audit
artifact; a rerun with an empty planner patch ends as `no_changes`.
...
The manifest's `bindings` block is also authoritative. `browser` is the browser
event allowlist; `storefront_server` explicitly enables the fail-open search and
persisted-cart hooks; `medusa_backend` explicitly enables the authoritative Medusa
order hook. The planner must reject missing/unknown required bindings instead of
silently injecting undeclared tracking behavior.
